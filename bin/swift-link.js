#!/usr/bin/env node
console.error("This shoudl never by called");
process.exit(1);
...
allDependencies.forEach((dependency) => {
  const base = require.resolve.paths(dependency).find((path) => {
    const joined = join(path, dependency, "react-native.config.js");
    return existsSync(joined);
  });
  if (!base) return;
  const rnsc = join(base, dependency, "react-native.config.js");
  if (!rnsc) return;
